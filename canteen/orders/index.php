    ...
    <div class="card card-outline card-primary rounded-0 shadow">
        ...
        <div class="card-body">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php 
                    // $orders = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.firstname, ', ',c.lastname,' ',COALESCE(c.middlename,'')) as client from `order_list` o inner join student_list c on o.client_id = c.id where o.canteen_id = '{$_settings->userdata('id')}' and date_format(o.date_created,'%Y-%m') = '{$month}' order by unix_timestamp(o.date_created) desc ");
                    // while($row = $orders->fetch_assoc()):


                    $i = 1;
                    $orders = $conn->query("SELECT * FROM `order_list` where canteen_id = '{$_settings->userdata('id')}' order by `status` asc,unix_timestamp(date_created) desc ");
                    while($row = $orders->fetch_assoc()):
                    ?>
                    <tr>
                        ...
                        <td class="px-2 py-1 align-middle text-right"><i class="nav-icon fas fa-rupee-sign"></i>  <?= format_num($row['total_amount']) ?></td>
