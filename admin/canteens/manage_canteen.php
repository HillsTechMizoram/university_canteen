<?php 
if(isset($_GET['id'])){
	$canteen = $conn->query("SELECT * FROM canteen_list where id ='".$_GET['id']."'");
	if($canteen->num_rows > 0){
		$res = $canteen->fetch_array();
		foreach($res as $k =>$v){
			$$k = $v;
		}
	}else{
		echo '<script> alert("Unknown Canteen"); location.replace("./?page=canteens")</script>';
	}
				...
				<form action="" id="manage-user">	
					...
					<div class="row">
						<div class="form-group col-md-6">
							<label for="canteen_name" class="control-label">Canteen Name</label>
							<input type="text" id="canteen_name" autofocus name="canteen_name" class="form-control form-control-sm form-control-border" value="<?= isset($canteen_name) ? $canteen_name : "" ?>" required>
						</div>
						<div class="form-group col-md-6">
							<label for="canteen_owner" class="control-label">Shop Owner Fullname</label>
							<input type="text" id="canteen_owner" name="canteen_owner" class="form-control form-control-sm form-control-border" value="<?= isset($canteen_owner) ? $canteen_owner : "" ?>" required>
						</div>
						<div class="form-group col-md-6">
						...
						<div class="form-group col-md-6">
							<label for="canteen_dept_id" class="control-label">Shop Type</label>
							<select type="text" id="canteen_dept_id" name="canteen_dept_id" class="form-control form-control-sm form-control-border select2" required>
								<option value="" disabled selected></option>
								<?php 
								$types = $conn->query("SELECT * FROM `canteen_dept` where delete_flag = 0 and `status` = 1 ".(isset($canteen_dept_id) ? " or id = '{$canteen_dept_id}' " : "")." order by `name` asc ");
								while($row = $types->fetch_assoc()):
								?>
								<option value="<?= $row['id'] ?>" <?= isset($canteen_dept_id) && $canteen_dept_id == $row['id'] ? 'selected' : '' ?>><?= $row['name'] ?></option>
								<?php endwhile; ?>
							</select>
						</div>
					</div>
