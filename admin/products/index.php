        ...
        <div class="w-100 overflow-auto">
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT p.*,v.code, v.canteen_name as `canteen` from `product_list` p inner join canteen_list v on p.canteen_id = v.id where p.delete_flag = 0 order by p.`name` asc ");
					...
					?>
						<tr>
							...
							<td>
								<div class="border-bottom text-truncate"><?= $row['code'].'-'.$row['canteen'] ?></div>
								<div class="text-truncate"><?= $row['name'] ?></div>
							</td>
							<td class="text-right"><i class="nav-icon fas fa-rupee-sign"></i>  <?php echo format_num($row['price']) ?></td>
