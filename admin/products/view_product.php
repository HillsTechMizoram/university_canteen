...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT p.*, c.name as `category`,v.code, v.canteen_name as `canteen` from `product_list` p inner join category_list c on p.category_id = c.id inner join canteen_list v on p.canteen_id = v.id where p.id = '{$_GET['id']}' and p.delete_flag = 0 ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	...
	<dl>
		<dt class="text-muted">Canteen</dt>
