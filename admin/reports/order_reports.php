                    ...
                    <table class="table table-bordered table-stripped">
                        ...
                        <thead>
                            <tr class="">
                                ...
                                <th class="text-center align-middle py-1">Canteen</th>
                        ...
                        <tbody>
                            ...
                            $i = 1;
                            $total = 0;
                            $orders = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as client,concat(v.code, '-',v.canteen_name) as canteen from `order_list` o inner join student_list c on o.client_id = c.id inner join canteen_list v on o.canteen_id = v.id where date_format(o.date_created,'%Y-%m') = '{$month}' order by unix_timestamp(o.date_created) desc ");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="align-middle px-2 py-1"><?php echo ucwords($row['canteen']) ?></td>
                                    ...
                                    <td class="text-right align-middle px-2 py-1"><i class="nav-icon fas fa-rupee-sign"></i> <?php echo format_num($row['total_amount']) ?></td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
                        <tfoot>
                            <tr>
                                ...
                                <th class="text-right px-1 py-1 align-middel"><i class="nav-icon fas fa-rupee-sign"></i>  <?= format_num($total) ?></th>
                            </tr>
                        </tfoot>
                    </table>
                    </div>
