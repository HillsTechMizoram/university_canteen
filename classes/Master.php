...
Class Master extends DBConnection {
	...
	function save_shop_type(){
		...
		$check = $this->conn->query("SELECT * FROM `canteen_dept` where `name` = '{$name}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
		...
		if($check > 0){
			...
			if(empty($id)){
				$sql = "INSERT INTO `canteen_dept` set {$data} ";
			}else{
				$sql = "UPDATE `canteen_dept` set {$data} where id = '{$id}' ";
			}
	...
	function delete_shop_type(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `canteen_dept` set delete_flag = 1 where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function save_category(){
		...
		$check = $this->conn->query("SELECT * FROM `category_list` where `name` = '{$name}' and canteen_id = '{$canteen_id}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_product(){
		...
		$check = $this->conn->query("SELECT * FROM `product_list` where canteen_id = '{$canteen_id}' and `name` = '{$name}' and delete_flag = 0 ".(!empty($id) ? " and id != '{$id}'" : ""))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$canteens = $this->conn->query("SELECT * FROM `canteen_list` where id in (SELECT canteen_id from product_list where id in (SELECT product_id FROM `cart_list` where client_id ='{$this->settings->userdata('id')}')) order by `canteen_name` asc");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
		while($vrow = $canteens->fetch_assoc()):
			...
			$sql = "INSERT INTO `order_list` (`code`,`client_id`,`canteen_id`,`delivery_address`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
				...
				$data = "";
				$gtotal = 0 ;
				$products = $this->conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.client_id = '{$this->settings->userdata('id')}' and p.canteen_id = '{$vrow['id']}' order by p.name asc");
